clc;
drone = struct(); % Initialize an empty structure for the drone
groundStation = struct(); % Initialize an empty structure for the ground station
...
drone.operatorID = 'Operator1'; % Operator ID
...
for t = 0:smoothStep:simulationTime
    ...
    if distanceToGroundStation <= groundStation.range
        ...
        if authenticated
            ...
            % Define the words
            word1 = num2str(drone.ID);            word2 = num2str(drone.latitude);            word3 = num2str(drone.longitude);            word4 = num2str(drone.altitude);
